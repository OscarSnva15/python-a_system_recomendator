...
class Recomender:
    ...
    def show_distances_for_each_business_original_osc(self):
...
def main():
    ...
    #ahora pasar todos los codigos de clase de actividad satelite.
    codes_satelite =[461110, 465311, 812110, 463211, 461122, 722513, 467111, 461130, 461160, 311830, 311812, 722514, 461121, 464111, 722517, 561432, 467115, 811111, 722518, 722515, 722519, 621211, 332320, 461190, 813210, 465912, 466410, 468211, 531113, 713120, 811121]
    #codes_satelite =[461150, 463310, 811191, 611111, 611122, 467114, 722511, 465111, 713943, 321910]
    codes_support = [465311,
                    812110,
                    463211,
                    461122,
                    722513,
    ...
    # definimos excel para que genere reporte de distancias
    dfExcel = pd.DataFrame(
        columns=['Name_class_ue_satelite_business','count_ue_satelite_business','ratio','code_class_business']+list(codes_support),
        index=range(0)
    )

    for code in codes_satelite:
        bussines_snapshot = Recomender(df, code, time_window)
        print('*show_distances_for_each_bussines_in_code_class:\n',bussines_snapshot.show_distances_for_each_business())
        print('*report_accumulated_bussines_support*')
        row = bussines_snapshot.report_accumulated_business_support()
        dfExcel.loc[len(dfExcel)] = row

    # Guardamos el DataFrame en un archivo Excel
    with pd.ExcelWriter('../querys/distancias/reporte_distancias/report_final_accumulated_business.xlsx', engine='xlsxwriter') as writer: dfExcel.to_excel(writer, sheet_name='sheet1', index=True)
